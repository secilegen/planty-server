...
router.post("/get-support", (req, res, next) => {
  let bookingGlobal
    const {description, reasonWhy, user, expert, isOnline, isConfirmed, rating, image, date } = req.body;
  
    Booking.create({ description, reasonWhy, user, expert, isOnline, isConfirmed, rating, image, date })
      .then((response) => {
